...
static void generate_headers(void) {
	std::cout << "#include \"nbp_runtime.h\"" << std::endl;
}
...
static void generate_establish(void) {
	auto ast = builder::builder_context().extract_function_ast(establish_wrapper, "nbp__establish");
...
static void generate_destablish(void) {
	auto ast = builder::builder_context().extract_function_ast(destablish_wrapper, "nbp__destablish");
	block::eliminate_redundant_vars(ast);
	block::c_code_generator::generate_code(ast, std::cout);
...
static void generate_send(void) {
	auto ast = builder::builder_context().extract_function_ast(send_wrapper, "nbp__send");
	block::eliminate_redundant_vars(ast);
	block::c_code_generator::generate_code(ast, std::cout);	
}
...
static void generate_ingress_step(void) {
	auto ast = builder::builder_context().extract_function_ast(run_ingress_step_wrapper, "nbp__run_ingress_step");
	block::eliminate_redundant_vars(ast);
	block::c_code_generator::generate_code(ast, std::cout);	
}
...
static void generate_net_init(void) {
	auto ast = builder::builder_context().extract_function_ast(net_init_wrapper, "nbp__net_init");
	block::eliminate_redundant_vars(ast);
	block::c_code_generator::generate_code(ast, std::cout);	
}
...
static void generate_connection_layout(std::string fname) {
	...
	hoss << "#include \"nbp_data_queue.h\"" << std::endl;
	conn_layout.generate_struct_decl(hoss, "nbp__connection_t");
	net_state.generate_struct_decl(hoss, "nbp__net_state_t");
	hoss.close();
}
