...
#define IPC_MTU (1024)
int nbp__ipc_simulate_out_of_order = 0;
int nbp__ipc_simulate_packet_drop = 0;
...
char nbp__reuse_mtu_buffer[IPC_MTU];
...
int nbp__send_packet(char* buff, int len) {
	if (nbp__ipc_simulate_packet_drop) {
		int r = rand() % PACKET_DROP_CHANCE;
		if (r == 0) {
			//nbp__debug_packet(buff);
			return len;
		}
	}
	// Don't try to out of order if we already have on pending
	if (out_of_order_len == 0 && nbp__ipc_simulate_out_of_order) {
